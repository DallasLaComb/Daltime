...
- **Org Admins column:** shows `org_admin_count ?? 0` as a badge. `0` or `undefined` → grey badge showing "0". `> 0` → blue badge. The `?? 0` fallback ensures legacy orgs that lack the `org_admin_count` attribute in DynamoDB never render a blank badge.
...
interface Organization {
  ...
  org_admin_count: number | undefined; // maintained by org-admins Lambda; undefined for legacy orgs that predate the counter — always display as `?? 0`
}
...
| 4b  | Count badge shows "0" when org_admin_count is undefined | `org_admin_count: undefined`  | grey badge showing "0 Admins" (not blank)               |
...
- [x] ~~`org_admin_count` blank for legacy orgs~~ — fixed in story #348: interface widened to `number | undefined`, all template bindings use `?? 0` fallback
