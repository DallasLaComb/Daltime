...
export async function getEmployeeReverseLookup(
  ...
  if (!result.Item) return null;
  return result.Item as { employee_id: string; org_id: string; email: string };
}
...
export async function updateEmployee(
  ...
  if (fields.manager_id !== undefined) {
    names['#manager_id'] = 'manager_id';
    values[':manager_id'] = fields.manager_id;
    parts.push('#manager_id = :manager_id');
  }
